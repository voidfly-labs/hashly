export default {
  themeStorageKey: 'xxhash-theme',
  title: 'xxHash64 Hash Calculator | xxHash.dev',
  description:
    'Calculate xxHash32, xxHash64, xxHash3, and xxHash128 checksums in your browser. Instant non-cryptographic hashing — no uploads, no tracking.',
  canonicalUrl: 'https://www.xxhash.dev/',
  ogImage: 'https://www.xxhash.dev/og-image.png',
  siteName: 'xxHash.dev',
  author: 'xxHash.dev',
  brandUrl: 'https://www.xxhash.dev',
  brandTitleHtml: '<span>xx</span>Hash',
  skipLinkText: 'Skip to xxHash64 hash calculator',
  mainAriaLabel: 'xxHash64 hash calculator',
  pageH1Html: '<span>xxHash64</span> Hash Calculator',
  pageSubtitle: '// instant hashing — no data leaves your browser',
  sectionNoun: 'hash',
  legal: {
    domain: 'xxhash.dev',
    library: { name: 'hash-wasm', url: 'https://github.com/Daninet/hash-wasm', author: 'Daninet' },
    privacy: {
      title: 'Privacy Policy | xxHash.dev',
      description:
        'Privacy policy for xxHash.dev by Voidfly Labs. All hashing runs in your browser — no data is ever transmitted.',
      canonicalUrl: 'https://www.xxhash.dev/privacy',
    },
    terms: {
      title: 'Terms of Service | xxHash.dev',
      description: 'Terms of service for xxHash.dev by Voidfly Labs. Free to use, no account required, provided as-is.',
      canonicalUrl: 'https://www.xxhash.dev/terms',
    },
  },
};
