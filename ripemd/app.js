'use strict';
    ...
    const Hasher = {
      ...
      _call(cryptoApiId, encodedData) {
        if (!window.CryptoApi) throw new Error(`crypto-api global 'CryptoApi' not available. Ensure the bundle has loaded.`);
      ...
      async fromFileAll(file, onProgress, algos = ALGORITHMS) {
        ...
        return new Map(
          hashers.map(({ id, instance }) => [id, CryptoApi.encoder.toHex(instance.finalize())])
        );
      },
      ...
      generateRandom(algoId) {
        const byteLengths = {
          'RIPEMD-128': 16, 'RIPEMD-160': 20, 'RIPEMD-256': 32, 'RIPEMD-320': 40,
        };
